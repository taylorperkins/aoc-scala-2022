  ...
  with Utils
{
  ...
  def dijkstra(start: Vertex)(criteria: Vertex => Boolean): Int = {
    ...
    @tailrec
    def dijkstra0(currentNode: (Vertex, Int), considering: Map[Vertex, Int], visited: Set[Vertex]): Int = {
      if (criteria(currentNode._1)) currentNode._2
      else
  ...
  using(resource("src/main/resources/day12.txt")) { input => {
    ...
    implicit val grid: Grid = input.getLines.map(_.toCharArray).toArray
    val vertices = for {
